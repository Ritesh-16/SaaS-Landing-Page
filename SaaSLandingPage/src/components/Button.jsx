...
const Button =({ icon, children, href, containerClassName, onClick,markerFill}) => {
    ...
    const Inner = () => (
        <>
        <span className='relative flex items-center min-h-[60px] px-4 g4 rounded-2xl innner-before group-hover:before:opacity-100 overflow-hidden' >
    ...
    return href ? (
        <a className={clsx('relative p-0.5 g5 rounded-2xl shadow-500 group',containerClassName)} href={href}><Inner /></a>
    ) : (
